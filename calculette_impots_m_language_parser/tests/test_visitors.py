...
'''
    ...
    result = visit_parse_tree(parse_tree, m_to_ast.MLanguageVisitor())
    print(result)
    assert_equal(len(result), 1)
    assert_equal(result[0]['type'], 'regle')
    assert_equal(result[0]['name'], '999')
    assert_equal(result[0]['applications'], ['iliad', 'batch'])
    assert_equal(len(result[0]['formulas']), 1)
    assert_equal(result[0]['formulas'][0]['type'], 'formula')
    assert_equal(result[0]['formulas'][0]['name'], 'a')
    assert_equal(result[0]['formulas'][0]['expression']['type'], 'integer')
    assert_equal(result[0]['formulas'][0]['expression']['value'], 1)
